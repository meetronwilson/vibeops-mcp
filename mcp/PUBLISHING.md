...
Guide for maintainers on publishing new versions of VibeOps MCP Server to npm.
...
```bash
npm view @vibeops/vibeops
```

Visit: https://www.npmjs.com/package/@vibeops/vibeops
...
```bash
npm access grant read-write vibeops:developers @vibeops/vibeops
```
...
npm unpublish @vibeops/vibeops@1.0.0

# Unpublish entire package (not recommended)
npm unpublish @vibeops/vibeops --force
```
...
# Users install with:
npm install @vibeops/vibeops@beta
```
...
```bash
npm deprecate @vibeops/vibeops@1.0.0 "Please upgrade to 1.1.0"
```
